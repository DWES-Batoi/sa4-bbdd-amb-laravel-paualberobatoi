@extends('layouts.app')
@section('content')
<h1 class="text-2xl font-bold mb-4 text-blue-800">{{ __('Editar Resultat / Dades Partit') }}</h1>
<form action="{{ route('partits.update', $partit) }}" method="POST" class="bg-white p-6 shadow rounded">
    ...
    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded">{{ __('Actualitzar Partit') }}</button>
</form>
@endsection