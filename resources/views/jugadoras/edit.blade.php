@extends('layouts.app')
@section('content')
<h1 class="text-2xl font-bold mb-4">{{ __('Editar Jugadora') }}: {{ $jugadora->nom }}</h1>
<form action="{{ route('jugadoras.update', $jugadora) }}" method="POST" class="bg-white p-6 shadow rounded">
    ...
    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">{{ __('Guardar Canvis') }}</button>
</form>
@endsection